...
def solveProject(prjCfgPath, schFilePath, opnFilePath, netFilePath, resultFilePath, learning=True, evaluate=False):
    ...
    if evaluate:
        ...
        res = []
        res.append(lns(copy.deepcopy(cvrptw), copy.deepcopy(cvrptw_best), mipgap=0.1, timelimit=120, degree_of_destruction=0.4, iteration=10, shuffle="RANDOM_LNS", accept_rate=1.01))
        res.append(lns(copy.deepcopy(cvrptw), copy.deepcopy(cvrptw_best), mipgap=0.1, timelimit=120, degree_of_destruction=0.4, iteration=10, shuffle="RANDOM_NO_DISJOINT", accept_rate=1.01))
        # res.append(lns(copy.deepcopy(cvrptw), copy.deepcopy(cvrptw_best), mipgap=0.1, timelimit=120, degree_of_destruction=0.4, iteration=10, shuffle="DISTANCE_DESCEND_NO_DISJOINT", accept_rate=1.01))
        # res.append(lns(copy.deepcopy(cvrptw), copy.deepcopy(cvrptw_best), mipgap=0.1, timelimit=120, degree_of_destruction=0.4, iteration=10, shuffle="DISTANCE_DESCEND", randomness=0.6, disjoint=False, accept_rate=1.01))
        return res
