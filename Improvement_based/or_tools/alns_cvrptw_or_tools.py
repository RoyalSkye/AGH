...
def by_vehicle_destroy(cvrptw, degree_of_destruction=0.1, mipgap=0.1, timelimit=300, start=0, shuffle="DISTANCE_DESCEND"):
    ...
    # Construct model
    mdl = construct_or_tools_model(cvrptw, x_fixed)

    cvrptw.or_tools_solver = cp_model.CpSolver()
    cvrptw.or_tools_solver.parameters.log_search_progress = False
    cvrptw.or_tools_solver.parameters.max_time_in_seconds = timelimit
    cvrptw.or_tools_solver.parameters.num_search_workers = 32
    status = cvrptw.or_tools_solver.Solve(mdl)

    if status == 0:
        return None, end
    else:
        return cvrptw.or_tools_solver, end
...
def by_vehicle_destroy_randomness(cvrptw, degree_of_destruction=0.1, randomness=0.6, mipgap=0.1, timelimit=300, start=0, shuffle="DISTANCE_DESCEND", disjoint=True):
    """
...
def construct_cplex_model(cvrptw, x_fixed):
...
def by_vehicle_destroy_with_learning(cvrptw, destroy_id, mipgap=0.1, timelimit=300):
    ...
    if cvrptw.lns_constraints is not None:
        # Remove previous lns_cts
        cur_t = time.time()
        for constraint in cvrptw.lns_constraints:
            constraint.OnlyEnforceIf(cvrptw.false)
...
def add_constraints_learning_or_tools(mdl, x, t, cvrptw):
    """
    Add constraints to Cplex Model.
    """
    print("[*] add_constraints_learning")
    ...
    for i in customers:
        for f in fleets:
            mdl.Add(sum(x[i, j, k, f] for j in nodes_F[f] for k in vehicles[f] if i != j) == 1)
    ...
    for f in range(1, 1 + F):
        for h in customers:
            for k in vehicles[f]:
                mdl.Add((sum(x[i, h, k, f] for i in nodes_F[f] if i != h) -
    ...
    for f in range(1, 1 + F):
        ...
        for i in nodes_F[f]:
            for j in nodes_F[f]:
                if i != j:
                    for k in vehicles[f]:
                        mdl.Add(t[i, f] + tau[i, j, f] - (10 ** 8) * (1 - x[i, j, k, f]) <= t[j, f])
    ...
    #         for j in nodes_F[f]:
    #             if i != j:
    #                 x_fleet[i, j, f] = mdl.NewBoolVar('x_fleet[%i,%i,%i]' % (i, j, f))
    #                 mdl.Add(sum(x[i, j, k, f] for k in vehicles[f]) >= 1).OnlyEnforceIf(x_fleet[i, j, f])
    #                 mdl.Add(t[i, f] + tau[i, j, f] <= t[j, f]).OnlyEnforceIf(x_fleet[i, j, f])

    for i in nodes_F[1]:
        mdl.Add(start_ea[i] <= t[i, 1])

    # 5.TW end - 10
    for i in nodes_F[10]:
        mdl.Add(t[i, 10] <= start_la[i]-S_time[10][i-1])
    ...
    for i in cvrptw.args["customers"]:
        for precedence_item in cvrptw.args["precedence"][i]:
            mdl.Add(t[i, precedence_item[0]] + S_time[precedence_item[0]][i - 1] <= t[i, precedence_item[1]])

    for i in cvrptw.args["customers"]:
        for precedence_item in cvrptw.args["precedence"][i]:
            b = 1 if S_time[precedence_item[0]][i - 1] != 0 else 0
            if b != 0:
                cvrptw.data["e"].append(
                    [count, cvrptw.id_map[i, precedence_item[0]], 1 / S_time[precedence_item[0]][i - 1]])
                cvrptw.data["e"].append(
                    [count, cvrptw.id_map[i, precedence_item[1]], -1 / S_time[precedence_item[0]][i - 1]])
            else:
                cvrptw.data["e"].append([count, cvrptw.id_map[i, precedence_item[0]], 1])
                cvrptw.data["e"].append([count, cvrptw.id_map[i, precedence_item[1]], -1])
            cvrptw.data["c"].append([0, b, 0, 1])
...
def add_constraints_or_tools(mdl, x, t, cvrptw):
    """
    Add constraints to Cplex Model.
    """
    n = len(cvrptw.args["customers"])
...
def construct_or_tools_model(cvrptw, x_fixed, loading_path=None):
    start_time = time.time()

    if cvrptw.model is None:
        cvrptw.model = cp_model.CpModel()
        mdl = cvrptw.model

        cur_t = time.time()
        # variables
        x = {}
        for x_key in cvrptw.visit.keys():
            x[x_key] = mdl.NewBoolVar('x[%i,%i,%i,%i]' % (x_key[0], x_key[1], x_key[2], x_key[3]))
            mdl.AddHint(x[x_key], cvrptw.visit[x_key])
        t = {}
        for t_key in cvrptw.time.keys():
            t[t_key] = mdl.NewIntVar(-3600, 10 ** 6, 't[%i,%i]' % (t_key[0], t_key[1]))
            mdl.AddHint(t[t_key], cvrptw.time[t_key])
        cvrptw.x = x
        cvrptw.t = t
        cvrptw.true = mdl.NewConstant(1)
        cvrptw.false = mdl.NewConstant(0)
        print("Construct x {}, t {} within {}s".format(len(x), len(t), time.time() - cur_t))
        ...
        if cvrptw.learning:
            print(">> Learning ...")
            if not loading_path:
                mdl = add_constraints_learning_or_tools(mdl, x, t, cvrptw)
            else:
                mdl = add_constraints_or_tools(mdl, x, t, cvrptw)
        ...
        cur_t = time.time()
        n = len(cvrptw.args["customers"])
        mdl.Minimize(sum(cvrptw.args["distance"][i, j] * x[i, j, k, f] for i, j, k, f in cvrptw.visit) -
                     sum(x[n + 2 * f - 1, n + 2 * f, k, f] for f in cvrptw.args["fleets"] for k in
                         cvrptw.args["vehicles"][f]))
        print("Construct Obj within {}s".format(time.time() - cur_t))
    ...
    cur_t = time.time()
    pre_x = cvrptw.visit
    # fix part of variable by add_constraints
    # cvrptw.lns_constraints = mdl.add_constraints([cvrptw.x[key] == pre_x[key] for key in x_fixed])
    cvrptw.lns_constraints = []
    for key in x_fixed:
        cvrptw.lns_constraints.append(mdl.Add(cvrptw.x[key] == pre_x[key]))
        cvrptw.lns_constraints[-1].OnlyEnforceIf(cvrptw.true)
    print("Add New LNS Constraints within {}s".format(time.time() - cur_t))
    # print("constraints: {}, variables: {}".format(mdl.number_of_constraints, mdl.number_of_variables))
