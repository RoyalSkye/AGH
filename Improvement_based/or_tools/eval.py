...
import random
from models import *
from projectSolver_learning import solveProject, extract_v_feature, extract_k_f_feature
from alns_cvrptw_or_tools import by_vehicle_destroy_with_learning, construct_cplex_model, construct_or_tools_model
...
def evaluate(args, plot=False):
    # Load model
    print(f">> strategy: {args.strategy}, destroy_degree: {args.destroy_degree}, timelimit: {args.timelimit}, pre_timelimit: {pre_timelimit}")
    ...
    for _, file in enumerate(files):
        ...
        # res = solveProject(args.prjCfgPath, path, args.opnFilePath, args.netFilePath, args.resultFilePath, learning=False, evaluate=False, strategy=args.strategy, pre_timelimit=pre_timelimit)
        ...
        t1 = time.time()
        mdl = construct_or_tools_model(cvrptw, x_fixed=None, loading_path=loading_path)  # c, e
        ...
        for iteration in range(args.iter):
            ...
            create_datasets(shared_data, unshared_data, file_map, output_folder=args.data_folder, file_count=file_count, shared_data=False)
            ...
            print("Model decides to destroy {}/{}={:.2f}% vehicle".format(len(destroy_id), len(cvrptw.vehicle_fleet), len(destroy_id) / len(cvrptw.vehicle_fleet) * 100))
            solution = by_vehicle_destroy_with_learning(cvrptw, destroy_id, mipgap=args.mipgap, timelimit=args.timelimit)

            if int(time.time() - start_time - extra_time) > pre_timelimit:
                print(f">> Time out: {int(time.time() - start_time - extra_time)}s")
                break
...
if __name__ == '__main__':
    ...
    # Data parameters
    parser.add_argument('--data_folder', default="./data/20_Test1", help='folder with data files')
    ...
    parser.add_argument('--checkpoint', default="./models/checkpoint_FT_4.pth.tar", help='path to checkpoint, None if none.')
    ...
    parser.add_argument('--iter', type=int, default=100, help='LNS run __ iterations for each instance.')
    parser.add_argument('--mipgap', type=float, default=0.1, help='hyper-parameter for CPLEX.')
    parser.add_argument('--timelimit', type=int, default=25, help='hyper-parameter for CPLEX.')
