...
def generate_agh_data(dataset_size, agh_size, fleet_size=10):
    CAPACITIES = {10: 20., 20: 30., 50: 40., 100: 50., 200: 60., 300: 70.}
    n_gate, n_hour, n_min, prob = 91, 24, 60, np.load('./problems/agh/arrival_prob.npy')
    loc = 1 + np.random.choice(n_gate, size=(dataset_size, agh_size))
    arrival = 60 * np.random.choice(n_hour, size=(dataset_size, agh_size),
                                    p=prob) + np.random.randint(0, n_min, size=(dataset_size, agh_size))
    stay = torch.tensor([30, 34, 33]).repeat(dataset_size, 1)
    type_ = torch.tensor(np.random.randint(0, 3, size=(dataset_size, agh_size)), dtype=torch.long)
    departure = torch.tensor(arrival) + torch.gather(stay, 1, type_)
    demand = np.random.randint(1, 10, size=(dataset_size, fleet_size, agh_size)) / CAPACITIES[agh_size]
    return list(zip(loc.tolist(), arrival.tolist(), departure.tolist(), type_.tolist(), demand.tolist()))


def generate_gaussian_agh_data(dataset_size, agh_size, fleet_size=10):
    CAPACITIES = {10: 20., 20: 30., 50: 40., 100: 50., 200: 60., 300: 70.}
    n_gate, n_hour, n_min, prob = 91, 24, 60, np.load('./problems/agh/arrival_prob.npy')
    print(prob)
    prob = torch.softmax(torch.randn(24), dim=0).numpy()
    print(prob)
    loc = 1 + np.random.choice(n_gate, size=(dataset_size, agh_size))
    arrival = 60 * np.random.choice(n_hour, size=(dataset_size, agh_size),
                                    p=prob) + np.random.randint(0, n_min, size=(dataset_size, agh_size))
    ...
    type_ = torch.tensor(np.random.randint(0, 3, size=(dataset_size, agh_size)), dtype=torch.long)
    departure = torch.tensor(arrival) + torch.gather(stay, 1, type_)
    demand = np.random.normal(5, 2.5, size=(dataset_size, fleet_size, agh_size))
    demand = demand.tolist()
    for d in range(dataset_size):
        for f in range(fleet_size):
            for a in range(agh_size):
                demand[d][f][a] = 9 if demand[d][f][a] > 9 else demand[d][f][a]
                demand[d][f][a] = 1 if demand[d][f][a] < 1 else demand[d][f][a]
                demand[d][f][a] /= CAPACITIES[agh_size]
    return list(zip(loc.tolist(), arrival.tolist(), departure.tolist(), type_.tolist(), demand))


def generate_poisson_agh_data(dataset_size, agh_size, fleet_size=10):
    CAPACITIES = {10: 20., 20: 30., 50: 40., 100: 50., 200: 60., 300: 70.}
    n_gate, n_hour, n_min, prob = 91, 24, 60, np.load('./problems/agh/arrival_prob.npy')
    print(prob)
    prob = np.random.poisson(4, 24)
    prob = prob / sum(prob)
    print(prob)
    loc = 1 + np.random.choice(n_gate, size=(dataset_size, agh_size))
    arrival = 60 * np.random.choice(n_hour, size=(dataset_size, agh_size),
    ...
    type_ = torch.tensor(np.random.randint(0, 3, size=(dataset_size, agh_size)), dtype=torch.long)
    departure = torch.tensor(arrival) + torch.gather(stay, 1, type_)
    demand = np.random.poisson(5, size=(dataset_size, fleet_size, agh_size))
    demand = demand.tolist()
    for d in range(dataset_size):
        for f in range(fleet_size):
            for a in range(agh_size):
...
if __name__ == "__main__":
    ...
    parser.add_argument('--graph_sizes', type=int, nargs='+', default=[20, 50, 100, 200, 300],
        ...
        for distribution in distributions or [None]:
            for graph_size in opts.graph_sizes:
                ...
                if problem == 'tsp':
                    ...
                    dataset = generate_agh_data(opts.dataset_size, graph_size)
                    # dataset = generate_gaussian_agh_data(opts.dataset_size, graph_size)
                    # dataset = generate_poisson_agh_data(opts.dataset_size, graph_size)
                else:
