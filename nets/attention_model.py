...
class AttentionModel(nn.Module):
    ...
    def __init__(self,
        ...
        if self.is_agh:
            with open('problems/agh/fleet_info.pkl', 'rb') as f:
                # {'order': [1, 2, 4, 8, 3, 5, 7, 9, 6, 10], solved by this fleet order
                # 'precedence': {1: 0, 2: 1, 4: 1, 8: 1, 3: 2, 5: 2, 7: 2, 9: 2, 6: 3, 10: 4},
                # 'duration': {1: [0.0, 0.0, 0.0], ...]},
                # 'next_duration': {4: [0.0, 0.0, 0.0], ...}} the least time needed for fleets whose precedence less than that of key,
                #  e,g,. no fleet precedence is less than 4 (the last precedence level)
        ...
        if self.is_vrp or self.is_orienteering or self.is_pctsp:
            ...
            self.fleets_embedding = nn.Embedding(10, embedding_dim)  # 10 fleet
            step_context_dim = embedding_dim + 2  # remained_capacity, cur_free_time
            if self.wo_time or self.rnn_time:
                node_dim = 1  # demand
            else:
                node_dim = 3  # demand, tw_left, tw_right
            self.loc_embedding = nn.Embedding(92, embedding_dim)  # 91 gates + 1 depot in the airport
        else:  # TSP
        ...
        if self.rnn_time:
            self.time_embed = nn.Linear(2, embedding_dim)
    ...
    def forward(self, input, return_pi=False):
        """
        :param input: {
                'loc': x['loc'], # [batch_size, graph_size]
                'demand': x['demand'][:, f - 1, :],  # [batch_size, graph_size]
                'distance': model.distance.expand(x['loc'].size(0), len(model.distance)),  # [batch_size, len(distance)]
                'duration': torch.gather(duration, 1, x['type']),  # [batch_size, graph_size]
                'tw_right': tw_right,  # [batch_size, graph_size+1]
                'tw_left': tw_left,  # [batch_size, graph_size+1]
                'fleet': torch.full((x['loc'].size(0), 1), f - 1)  # [batch_size, 1]
                }
        ...
        # _log_p: (batch_size, steps=graph_size+, graph_size) pi: (batch_size, steps=graph_size+)
    ...
    def precompute_fixed(self, input, fleet=None):
        # TODO: combine with beam search, add 'fleet id' as one input in beam search
    ...
    def _init_embed(self, input):
        ...
        if self.is_vrp or self.is_orienteering or self.is_pctsp:
            ...
            init_embed = self.loc_embedding(torch.cat((torch.zeros_like(input['loc'][:, :1]), input['loc']), dim=1))  # [batch_size, graph_size+1, embedding_dim]
            if self.wo_time:
                fea_embed = self.init_embed(input['demand'][:, :, None].float())  # [batch_size, graph_size, embedding_dim]
            elif self.rnn_time:
                fea_embed = self.init_embed(input['demand'][:, :, None].float())
                batch_size, graph_size, embed_size = fea_embed.size()
                if self.pre_tw is None:
                    self.pre_tw = (torch.zeros(batch_size * graph_size, embed_size).float().to(fea_embed.device),
                                   torch.zeros(batch_size * graph_size, embed_size).float().to(fea_embed.device))
                time_embed = self.time_embed(torch.cat((input['tw_left'][:, 1:, None] / 1440,
                                                        input['tw_right'][:, 1:, None] / 1440), -1).float())  # [batch_size, graph_size, embedding_dim]
                h_1, c_1 = self.rnn(time_embed.view(-1, embed_size), self.pre_tw)
                h_1 = h_1.view(batch_size, graph_size, embed_size)
                fea_embed = fea_embed + h_1
                h_1 = h_1.view(-1, embed_size)
                self.pre_tw = (h_1, c_1)
            else:
                fea_embed = self.init_embed(torch.cat((input['demand'][:, :, None],
                                                       input['tw_left'][:, 1:, None] / 1440,
    ...
    def _inner(self, input, embeddings):
        ...
        while not (self.shrink_size is None and state.all_finished()):
            ...
            # log_p/mask: (batch_size, 1, graph_size + 1)
    ...
    def sample_many(self, input, batch_rep=1, iter_rep=1):
        # TODO:  _inner output has been change, adjust the code where use sample_many (in eval.py)
        """
    ...
    def _get_log_p(self, fixed, state, normalize=True):
        ...
        # Compute query = context node embedding [batch_size, 1, embedding_dim]
